...
namespace pryCafeteriaEscolar
{
    public partial class Ventas : UserControl
    {
        ...
        private void btnRegistrar_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrWhiteSpace(txtProduc.Text) ||
                string.IsNullOrWhiteSpace(txtCodgProduc.Text) ||
            ...
            {
                MessageBox.Show("Por favor, completa los datos del producto (Nombre, Código, Precio, Cantidad, Recibe del cliente, Fecha, Cambio).",
            ...
            string lineaProducto = $"{txtProduc.Text}";
        ...
        private void txtProduc_TextChanged(object sender, EventArgs e)
        {
            ...
            // Cambia  cadena de conexión
            string conexion = "Server=TU_PC\\SQLEXPRESS;Database=pryCafeteriaEs;Integrated Security=True";

            using (SqlConnection con = new SqlConnection(conexion))
            {
                con.Open();
                string query = "SELECT Nombre FROM Productos"; // Tu tabla
                using (SqlCommand cmd = new SqlCommand(query, con))
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        coleccion.Add(reader["Nombre"].ToString());
                    }
                }
            }
